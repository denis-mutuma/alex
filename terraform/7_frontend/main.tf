...
terraform {
  ...
  # Partial config: use -backend-config (CI) or terraform init -backend=false (local state).
  backend "s3" {}
...
# Reference Part 5 / Part 6 state: local .tfstate files (default) or same S3 bucket as CI (set var.remote_state_bucket)
data "terraform_remote_state" "database_s3" {
  count   = var.remote_state_bucket != "" ? 1 : 0
  backend = "s3"
  config = {
    bucket = var.remote_state_bucket
    key    = "alex/5_database/terraform.tfstate"
    region = var.aws_region
  }
}
...
data "terraform_remote_state" "database_local" {
  ...
  backend = "local"
  config = {
    path = "${path.module}/../5_database/terraform.tfstate"
  }
}
...
data "terraform_remote_state" "agents_s3" {
  ...
  config = {
    bucket = var.remote_state_bucket
    key    = "alex/6_agents/terraform.tfstate"
    region = var.aws_region
  }
}
...
data "terraform_remote_state" "agents_local" {
  ...
  backend = "local"
  config = {
    path = "${path.module}/../6_agents/terraform.tfstate"
  }
}

locals {
  database = var.remote_state_bucket != "" ? data.terraform_remote_state.database_s3[0] : data.terraform_remote_state.database_local[0]
  agents   = var.remote_state_bucket != "" ? data.terraform_remote_state.agents_s3[0] : data.terraform_remote_state.agents_local[0]
    ...
    Statement = [
      {
        ...
        Resource = local.database.outputs.aurora_cluster_arn
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = local.database.outputs.aurora_secret_arn
      }
    ]
  })
}
    ...
    Statement = [
      {
        ...
        Resource = local.agents.outputs.sqs_queue_arn
      }
    ]
  })
}
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      AURORA_CLUSTER_ARN = local.database.outputs.aurora_cluster_arn
      AURORA_SECRET_ARN  = local.database.outputs.aurora_secret_arn
      AURORA_DATABASE    = local.database.outputs.database_name
      DEFAULT_AWS_REGION = var.aws_region

      # SQS configuration from Part 6
      SQS_QUEUE_URL = local.agents.outputs.sqs_queue_url
