terraform {
  ...
  # Partial S3 backend: use terraform init -backend=false for local-only state, or
  # -backend-config=... after creating the bootstrap bucket (see terraform/0_bootstrap).
  backend "s3" {}
