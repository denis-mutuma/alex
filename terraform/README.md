...
### GitHub Actions / S3 remote state

 Stacks declare a partial `backend "s3" {}` block. For **local-only** use:

```bash
terraform init -backend=false
```

For **CI** and shared state, apply `terraform/0_bootstrap` once, migrate state to the bucket (see `terraform/0_bootstrap/README.md`), and set `remote_state_bucket` for Part 7 when reading Part 5/6 state from S3.
