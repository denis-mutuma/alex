...
def terraform_state_params():
    """S3 remote state (optional). If unset, Terraform uses local state (-backend=false)."""
...
def deploy_terraform():
    ...
    run_command(plan_cmd, cwd=terraform_dir, env=tf_env)

    print("\n  Applying deployment...")
    print("  Creating AWS resources...")
    run_command(
        ["terraform", "apply", "-input=false", "-auto-approve"], cwd=terraform_dir, env=tf_env
    )
